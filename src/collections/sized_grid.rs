...
impl<T, const W: usize, const H: usize> SizedGrid<T, W, H> {
    ...
    /// # Examples
    /// ```rust
    /// let a = SizedGrid::from([
    ///     [1, 2],
    ///     [3, 4]
    /// ]);
    ///
    /// let mut b = SizedGrid::from([
    ///     [2, 1],
    ///     [4, 3],
    /// ]);
    ///
...
impl<T: Copy, const W: usize, const H: usize> SizedGrid<T, W, H> {
    ...
    /// # Examples
    /// ```rust
    /// let grid = SizedGrid::<i32, 2, 2>::from([
    ///     [None, None],
    ///     [None, None],
    /// ]);
    ///
    /// assert_eq!(grid, SizedGrid::<i32, 2, 2>::new());
    /// ```
    ...
    /// ]);
    ///
    /// assert_eq!(grid, SizedGrid::<_, 2, 2>::new_with(true));
    /// ```
